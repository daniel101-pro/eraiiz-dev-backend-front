...
export default function SplashCursor({
  ...
  useEffect(() => {
    ...
    function generateColor(): ColorRGB {
      // Extremely subtle light green with very high transparency
      const baseGreen = 0.4; // Reduced light green base
      const variation = 0.05 * Math.random() - 0.025; // Minimal variations
      const opacity = Math.random() * 0.08 + 0.02; // Extremely high transparency (2% to 10%)
      
      // Return an extremely transparent light green
      return {
        r: 0,
        g: (baseGreen + variation) * opacity,
        b: 0
      };
    }

    function HSVtoRGB(h: number, s: number, v: number): ColorRGB {
      // We're not using HSV anymore, but keeping for type compatibility
